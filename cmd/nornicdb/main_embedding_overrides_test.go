...
func TestApplyServeEmbeddingOverrides(t *testing.T) {
	for _, key := range []string{"PROVIDER", "MODEL", "API_URL", "API_KEY", "MODE", "DIMENSIONS", "CACHE_SIZE", "GPU_LAYERS", "ENABLED"} {
	...
	for _, tc := range []struct {
		...
		{name: "environment overrides YAML", env: map[string]string{"PROVIDER": "openai", "MODEL": "env-model", "API_URL": "http://127.0.0.1:11436", "API_KEY": "env-fixture", "MODE": "text", "DIMENSIONS": "512", "CACHE_SIZE": "500", "GPU_LAYERS": "4", "ENABLED": "false"}, want: settings{"openai", "env-model", "http://127.0.0.1:11436", "env-fixture", "text", 512, 500, 4, false}},
		{name: "CLI overrides environment and YAML including zero false and empty", env: map[string]string{"PROVIDER": "openai", "API_KEY": "env-fixture", "MODE": "contextualized", "GPU_LAYERS": "4"}, flags: []string{"--embedding-provider=local", "--embedding-model=bge-m3", "--embedding-url=http://localhost:11434", "--embedding-key=", "--embedding-mode=text", "--embedding-dim=1024", "--embedding-cache=0", "--embedding-gpu-layers=0", "--embedding-enabled=false"}, want: settings{"local", "bge-m3", "http://localhost:11434", "", "text", 1024, 0, 0, false}},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			m := cfg.Memory
			got := settings{m.EmbeddingProvider, m.EmbeddingModel, m.EmbeddingAPIURL, m.EmbeddingAPIKey, m.EmbeddingMode, m.EmbeddingDimensions, m.EmbeddingCacheSize, m.EmbeddingGPULayers, m.EmbeddingEnabled}
