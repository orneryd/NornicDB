...
class FileWatchManager: ObservableObject {
    ...
    private var serverHost: String { ConfigManager.shared.hostAddress }
    private var serverPort: String { "7474" }  // HTTP port
    ...
    func authenticate(username: String, password: String) async -> Bool {
        guard let tokenURL = URL(string: "\(serverBaseURL)/auth/token") else {
            return false
        }
...
struct FileIndexerView: View {
    @ObservedObject var watchManager = FileWatchManager.shared
        ...
        HStack {
            ...
            if watchManager.connectionStatus == .authRequired {
                Button("Sign In") {
                    ...
                    authUsername = ConfigManager.shared.adminUsername
                    authPassword = ConfigManager.shared.adminPassword
                    showAuthSheet = true
                }
                .buttonStyle(.borderedProminent)
                .controlSize(.small)
            } else if watchManager.connectionStatus == .disconnected {
                Button("Retry") {
                    Task {
                        await watchManager.checkServerConnection()
                    }
    ...
    var authRequiredBanner: some View {
        VStack(spacing: 12) {
            ...
            Text("Sign in to enable semantic search and file indexing")
                .font(.callout)
                .foregroundColor(.secondary)
            
            Button("Sign In to NornicDB") {
                authUsername = ConfigManager.shared.adminUsername
                authPassword = ConfigManager.shared.adminPassword
                showAuthSheet = true
            }
...
class FileIndexerWindowController {
    ...
    func showWindow() {
        ...
        let contentView = FileIndexerView()
