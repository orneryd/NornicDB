...
func TestLoadFromEnv_ComprehensiveAdditionalEnvCoverage(t *testing.T) {
	...
	t.Setenv("NORNICDB_EMBEDDING_MODE", "contextualized")
	...
	if cfg.Memory.EmbeddingAPIURL != "http://embed-env" || cfg.Memory.EmbeddingAPIKey != "embed-env-key" || cfg.Memory.EmbeddingMode != "contextualized" || cfg.Memory.EmbeddingCacheSize != 999 {
...
func TestLoadFromEnv_EmbeddingWorkerNumWorkers(t *testing.T) {
	...
	os.Setenv("NORNICDB_EMBED_WORKER_NUM_WORKERS", "0")
	cfg = LoadFromEnv()
...
`), 0o600)
	...
	require.Equal(t, "contextualized", cfg.Memory.EmbeddingMode)
