...
func (db *DB) SetEmbedder(embedder embed.Embedder) {
	...
	db.embedQueue.SetOnEmbedded(func(node *storage.Node) {
		...
		// Trigger Auto-TLP inference after embedding generation.
		// This fixes server-side embeddings where OnStore was called before embedding existed.
		if node == nil || len(node.ChunkEmbeddings) == 0 || len(node.ChunkEmbeddings[0]) == 0 {
			return
		}

		// Parse qualified node ID to get database name and local ID
		dbName, localID, ok := splitQualifiedID(string(node.ID))
		if !ok {
			return // Unprefixed IDs not supported
		}

		// Get per-database inference service
		...
		suggestions, err := inferEngine.OnStore(ctx, localID, node.ChunkEmbeddings[0])
		if err != nil || len(suggestions) == 0 {
			return
		}

		// Create namespaced storage for edge creation
		edgeStorage := storage.NewNamespacedEngine(db.baseStorage, dbName)
		now := time.Now().UTC()
		for _, suggestion := range suggestions {
			...
			_ = edgeStorage.CreateEdge(edge) // Best effort
		}
		log.Printf("🔗 Auto-TLP created %d relationships for node %s", len(suggestions), node.ID)
	})
