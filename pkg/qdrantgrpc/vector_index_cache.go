...
func (c *vectorIndexCache) deleteCollection(collection string) {
	...
	for k, idx := range c.indexes {
		if k.collection == collection {
			// Best-effort memory reclaim for index implementations that keep tombstones
			// or large backing arrays (e.g. HNSW / brute arenas).
			if clearable, ok := idx.(interface{ Clear() }); ok {
				clearable.Clear()
			}
			delete(c.indexes, k)
		}
...
func (c *vectorIndexCache) search(ctx context.Context, collection string, dim int, dist qpb.Distance, vectorName string, query []float32, want int, minScore float64, ef int) []searchResult {
	if c == nil || want <= 0 {
		return nil
	}
	if len(query) != dim {
	...
	return idx.search(ctx, query, want, minScore, ef)
}
...
type vectorIndex interface {
	...
	search(ctx context.Context, query []float32, limit int, minScore float64, ef int) []searchResult
}
...
func newBruteVectorIndex(dim int, dist qpb.Distance) *bruteVectorIndex {
	return &bruteVectorIndex{
		dim:     dim,
		dist:    dist,
		vectors: make(map[string][]float32, 1024),
	}
}
...
func (v *bruteVectorIndex) upsert(id string, vec []float32) {
	...
	if v.dist == qpb.Distance_Cosine || v.dist == qpb.Distance_UnknownDistance {
		vector.NormalizeInPlace(cpy)
	}
	v.vectors[id] = cpy
}

func (v *bruteVectorIndex) search(ctx context.Context, query []float32, limit int, minScore float64, ef int) []searchResult {
	...
	for id, cand := range v.vectors {
		...
		score := scoreVectorIndex(v.dist, q, cand)
...
func (v *hnswVectorIndex) search(ctx context.Context, query []float32, limit int, minScore float64, ef int) []searchResult {
	if v == nil || v.hnsw == nil || limit <= 0 || len(query) != v.dim {
		return nil
	}
	results, err := v.hnsw.SearchWithEf(ctx, query, limit, minScore, ef)
	if err != nil {
		return nil
	}
	out := make([]searchResult, 0, len(results))
	for _, r := range results {
		out = append(out, searchResult{ID: r.ID, Score: float64(r.Score)})
	}
	return out
}

func scoreVectorIndex(dist qpb.Distance, query []float32, cand []float32) float64 {
	switch dist {
	case qpb.Distance_Dot:
		return float64(vector.DotProductSIMD(query, cand))
	case qpb.Distance_Euclid:
		return vector.EuclideanSimilarity(query, cand)
