...
func TestUnwindMergeBatchSetSemantics(t *testing.T) {
	...
	changed, err := applyUnwindMergeChainSetAssignment(node, unwindSimpleSetAssignment{prop: "t", expr: "t"}, map[string]interface{}{"t": nil}, resolve, &setWrites{})
	require.NoError(t, err)
	...
	_, err = applyUnwindMergeChainSetAssignment(node, unwindSimpleSetAssignment{prop: "t", expr: "t"}, map[string]interface{}{"t": map[string]interface{}{"a": 1}}, resolve, &setWrites{})
	assert.Error(t, err)

	edge := &storage.Edge{ID: "e", Properties: map[string]interface{}{"a": int64(1)}}
	changed, err = applyUnwindMergeChainEdgeSetAssignment(edge, unwindSimpleSetAssignment{mergeMap: true, expr: "p"}, map[string]interface{}{"p": map[string]interface{}{"a": nil, "b": int64(2)}}, resolve, &setWrites{})
	require.NoError(t, err)
