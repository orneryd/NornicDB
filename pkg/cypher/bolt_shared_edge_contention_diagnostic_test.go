...
// Diagnostic scheduling experiment for shared-edge contention; no root-cause claim.
package cypher
...
// TestBoltSharedEdgeContentionDiagnostic keeps independent scope generations
// contending on one generation-independent edge, without per-round barriers.
func TestBoltSharedEdgeContentionDiagnostic(t *testing.T) {
	...
	prefix := fmt.Sprintf("shared-edge-contention-%d", time.Now().UnixNano())
	source, sink := prefix+"-source", prefix+"-sink"
	call := func(phase, q string, p map[string]any) []*neo4j.Record {
		t.Helper()
		r, err := sharedEdgeBoltCall(ctx, t, driver, phase, q, p)
		if err != nil {
			t.Fatal(err)
		}
		return r
	...
	defer func() {
		...
		defer done()
		_, err := sharedEdgeBoltCall(c, t, driver, "cleanup", `UNWIND $uids AS uid MATCH (f:Function {uid:uid}) DETACH DELETE f`, map[string]any{"uids": []string{source, sink}})
		if err != nil {
	...
	call("control_seed", concurrentRetractUpsertQuery, map[string]any{"rows": []map[string]any{control}})
	...
	call("target_seed", concurrentRetractUpsertQuery, map[string]any{"rows": []map[string]any{makeRow(0, 0)}})
	seed := tuples(prefix)
	if len(seed) != 1 {
	...
	for worker := 0; worker < workers; worker++ {
		go func(worker int) {
			...
			for round := 0; round < rounds; round++ {
				...
				_, err := sharedEdgeBoltCall(ctx, t, driver, phase+"/retract", concurrentRetractDeleteQuery, params)
				...
				_, err = sharedEdgeBoltCall(ctx, t, driver, phase+"/upsert", concurrentRetractUpsertQuery, map[string]any{"rows": []map[string]any{makeRow(worker, round)}})
	...
	call("designated_replay", concurrentRetractUpsertQuery, map[string]any{"rows": []map[string]any{row}})
	replay := tuples(prefix)
	call("duplicate_replay", concurrentRetractUpsertQuery, map[string]any{"rows": []map[string]any{row}})
	if len(replay) != 1 || !allowed[replay[0]] || !slices.Equal(replay, tuples(prefix)) {
		t.Fatal("duplicate replay altered tuple")
	}
	call("final_retract", concurrentRetractDeleteQuery, params)
	call("empty_replay", concurrentRetractDeleteQuery, params)
...
// around boltDiagnosticCall. Both helpers also recognize typed Outdated errors
// emitted by current localized conflicts. The predicate below preserves the
// legacy MERGE-only retry contract; UNWIND MATCH not-found stays terminal.
func sharedEdgeSnapshotConflict(err error, query string) bool {
...
func sharedEdgeBoltCall(ctx context.Context, t *testing.T, driver neo4j.DriverWithContext, phase, query string, params map[string]any) ([]*neo4j.Record, error) {
	for attempt := 0; ; attempt++ {
		...
		snapshot := sharedEdgeSnapshotConflict(err, query)
