...
func TestExecuteQueryAgainstStorage_DispatchCoverage(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "show_unsupported", cypher: "SHOW FOO", expectErr: true, errLike: "unsupported query type: SHOW"},
		{name: "drop_missing_index", cypher: "DROP INDEX idx_any", expectErr: true, errLike: "does not exist"},
		{name: "drop_missing_index_if_exists", cypher: "DROP INDEX idx_any IF EXISTS", expectErr: false},
