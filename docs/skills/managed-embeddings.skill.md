...
export NORNICDB_EMBEDDING_PROVIDER=ollama          # ollama | openai | local | voyage
...
embedding:
  enabled: true
  provider: ollama          # ollama | openai | local | voyage
  model: mxbai-embed-large
  url: http://localhost:11434
  dimensions: 1024
```

The OpenAI and Voyage providers also read `embedding.api_key` (or `NORNICDB_EMBEDDING_API_KEY`). Voyage also falls back to `VOYAGE_API_KEY`. The local provider resolves the model file inside `NORNICDB_MODELS_DIR`.
...
| `voyage` | `VOYAGE_API_KEY=pa-...` | Defaults to `https://api.voyageai.com`; set `NORNICDB_EMBEDDING_VOYAGE_MODE=contextualized` for provider-managed chunking |
