...
| `NORNICDB_SEARCH_RERANK_MAX_DOCUMENT_CHARS` | `2048` | Maximum characters (Unicode code points, not bytes) sent for one candidate (identifying properties + passage), so the same setting gives the same amount of text for Latin, Cyrillic or CJK content. Managed vector hits send the matched chunk extended with its neighbouring chunks up to this budget; lexical-only hits send a query-centered window. `NORNICDB_SEARCH_RERANK_MAX_DOCUMENT_BYTES` is deprecated: it is honoured when the new variable is unset and its value is read as a character count |
